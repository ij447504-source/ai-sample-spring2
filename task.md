...
- [ ] **T-6.2 SQL 기초 페이징 (Repository 직접 구현)**
  - [ ] H2의 `LIMIT`, `OFFSET`을 사용하는 네이티브 쿼리 작성 (`BoardRepository`)
  - [ ] `page` 파라미터를 컨트롤러에서 받아 동적 쿼리 호출 (0페이지부터 시작)
  - [ ] 서비스 레이어에서 비즈니스 로직(한글 메서드명) 구현
